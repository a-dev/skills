...
`scripts/audit.mjs` detects the package manager from `packageManager` and lockfiles. Multiple lockfile families are ambiguous. Never create a second lockfile. The audit also checks the profile format and versions, aliases, shared entry points and exports, layer declarations and ownership, color files and `color-scheme` mapping, commands, and CI ordering.

The contract resolver discovers the lint engine from Oxlint or ESLint config files and package scripts. It checks dependencies only when neither engine has a config or script. If both engines appear at the selected level, or neither appears, it defaults to ESLint. The compact profile's `lintEngine` overrides discovery.

Compact profiles declare their format and version and resolve through the same contract as legacy profiles. The resolver derives compact commands only from real package scripts and known package-manager metadata. Missing commands remain unverified.
